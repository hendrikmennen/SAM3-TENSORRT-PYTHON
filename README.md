...
This project provides a complete pipeline to run **SAM3** (Segment Anything Model 3) with **TensorRT**, enabling real-time performance on consumer-grade hardware.
...
The workflow is designed around FP16 TensorRT engines with dynamic shapes and explicit batching, supporting both **bounding box detection** and **mask segmentation** modes.
 ...
 * Download the original SAM3 weights:
<!-- end list -->
hf download facebook/sam3 --local-dir sam3

 * Run the ONNX export script (SAM3_PyTorch_To_Onnx.py is a self‑contained exporter):
<!-- end list -->
 ...
 * Original SAM3 implementation: facebook/sam3
 * TensorRT optimization techniques: jamjamjon/usls SAM3 scripts
